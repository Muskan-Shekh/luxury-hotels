...
const FilterSection = () => {
  ...
  const handleCountryRoute = (slug) => {
    route.push(`/country/${slug}`);
  };

  const filteredCountryData = countryData?.filter((country) =>
    country?.country.includes(countrySearch.toLowerCase())
  );
              ...
              <ul className="hotel-list">
                {filteredHotels.map((hotel, index) => {
                  return (
                    <li
                      key={index}
                      className="hotel-item border-b border-grayDark"
                    >
                      <h3
                        className="text-lg mb-0 uppercase h-name text-golden cursor-pointer"
            ...
            {countrySearch && (
              <ul className="hotel-list">
                {filteredCountryData?.map((hotel, index) => (
                  <li
                    key={index}
                    className="hotel-item border-b border-grayDark cursor-pointer"
                    onClick={() => handleCountryRoute(hotel?.country)}
                  >
                    <div className="flex items-center c-name gap-2">
                      <svg
                        stroke="currentColor"
