...
import ReviewForm from "../hotel_visit/ReviewForm";
import VoteForm from "../hotel_visit/VoteForm";
...
const Hotel_visit = ({ params }) => {
  ...
  const {
    request: requestNearby,
    response: responseNearby,
    loading,
  } = useRequest(true);
  ...
  const {
    request: requestExclusive_offer,
    response: responseExclusive_offer,
    clear: clearOffer,
  } = useRequest(true);
  const {
    request: requestReiview,
    response: responseReiview,
    clear: clearReiview,
  } = useRequest(true);
  ...
  const { hotel_highlight, hotel_facility, room_enimities } = useSelector(
...
const [currentPage, setCurrentPage] = useState(1);
  ...
  useEffect(() => {
    ...
    if (params) {
      requestfetch("GET", `${apis.GET_ALL_HOTELS_BY_COUNTRY}/${currentPage}`);
    }
  ...
  useMemo(() => {
    if (!new_fetch_hotel_info && responsefetch) {
      if (!offer_details) {
        requestExclusive_offer(
          "GET",
          `${apis.GET_EXCLUSIVEOFFERS_BY_HOTEL_ID}/${responsefetch.hotel._id}`
        );
      }
      if (!review_details) {
        requestReiview(
          "GET",
          `${apis.GET_HOTEL_REVIEWS}/${responsefetch.hotel._id}`
        );
      }
      clear();
  ...
  const NearByHotels = async () => {
    try {
      const response = await requestNearby(
        "POST",
        apis.NEARBY_BY_HOTEL,
        latLong
      );
  ...
  return (
    <>
      <section
        className="hotel-listing dashboard-section section-padding"
        style={{
      ...
      >
        <div className="container">
          <div
            className="content-rightBox white-bg"
            style={{
              backgroundImage: 'url("/new/assets/img/nominate-hotel-bg.png")',
              backgroundSize: "cover",
              backgroundPosition: "center center",
            ...
            <div className="row justify-between items-center">
              <div className="col-md-8">
                <h3 className="flex items-center">
                  {" "}
                  <span> {new_fetch_hotel_info?.hotel?.hotel_name}</span>
                  <StarRating
                    rating={new_fetch_hotel_info?.rating?.overall_rating}
                  />
                </h3>
                ...
                <div className="hotel-info">
                  ...
                  <span>
                    ...
                    <button
                      ...
                      className="font-semibold ps-2 text-golden hover:underline"
                    >
                      Show on Map
                    </button>
                  </span>
                ...
                <div className="hotel-info">
                  ...
                  <span>
                    {truncateText(new_fetch_hotel_info?.hotel?.description, 5)}
                    <button
                      onClick={() => SetShowMore(true)}
                      className="font-semibold text-golden ps-2 hover:underline"
                    >
                      Show More
                    </button>
                  </span>
              ...
              {ShowMore && (
                <PropertyDescription
                  about={new_fetch_hotel_info?.hotel.about}
                  spa_Wellnes={new_fetch_hotel_info?.hotel?.spa_wellness}
                  restuarent_bar={new_fetch_hotel_info?.hotel.restaurants_bars}
                  location={new_fetch_hotel_info?.hotel.location}
                  other_facility={new_fetch_hotel_info?.hotel.other_facility}
                  room_suite={new_fetch_hotel_info?.hotel.rooms_suites}
                  SetShowMore={SetShowMore}
                />
              )}

              <div className="col-md-4 flex justify-end">
                <button className="theme-btn g-btn me-3">like</button>
                <a
                  onClick={() =>
                ...
                <>
                  <div className="gallery-item last-grid-item relative cursor-pointer">
                    <img
                      ...
                      src={
                        remainingImages?.[0]?.startsWith("https")
                          ? remainingImages?.[0]
                          : `${BASEURL}/${remainingImages?.[0]}`
                      }
                    />
                    <div className="absolute inset-0 gird-info-number  flex items-center justify-center">
                      ...
                      >
                        See All {new_fetch_hotel_info?.hotel?.images?.length}{" "}
                        Photos
                      </span>
                    </div>
                  </div>
          ...
          <div
            className="content-rightBox white-bg mt-5"
            style={{
              backgroundImage: 'url("/new/assets/img/nominate-hotel-bg.png")',
              backgroundSize: "cover",
              backgroundPosition: "center center",
            ...
            <div className="row">
              <div className="col-md-8 dashHotel-listing">
                ...
                <div className="grid  service-outerBox p-3 bg-gray my-3 rounded-xl shadow-sm">
                  {hotel_highlight?.map(
                    (it) =>
                      new_fetch_hotel_info?.hotel?.hotel_highlights.includes(
                        it._id
                      ) && (
                ...
                <div className="grid  service-outerBox p-3 bg-gray my-3 rounded-xl shadow-sm">
                  {hotel_facility?.map(
                    (it) =>
                      new_fetch_hotel_info?.hotel?.hotel_facilities.includes(
                        it._id
                      ) && (
                        <div className="text-center serviceBox" key={it._id}>
                          <img
                ...
                <div className="grid  service-outerBox p-3 bg-gray my-3 rounded-xl shadow-sm">
                  {room_enimities?.map(
                    (it) =>
                      new_fetch_hotel_info?.hotel?.room_amenities.includes(
                        it._id
                      ) && (
                        <div className="text-center serviceBox" key={it._id}>
                          <img
                            src={`${BASEURL}/${it.amenityIcon}`}
                            className="mx-auto"
                            alt={it.amenity}
                          />
              ...
              <div className="col-md-4">
                <div
                  className="viewMapBox relative h-full"
                ...
                >
                  ...
                  {ViewModal && (
                    <ViewMap
                      ...
                      hotel_details_fetch={new_fetch_hotel_info?.hotel}
            ...
            <div className="row">
              ...
              <div className="col-md-3">
                <div className="text-center offerBlockRight space-y-3 p-3 min-h-[450px] special-card flex-1">
                  <h3 className="text-center text-xl text-golden">
                    EXCLUSIVE OFFER
                  </h3>
                  <svg
                    stroke="currentColor"
                  ...
                  <h4 className="uppercase md:text-xl">
                    {offer_details?.offer_name}
                  </h4>
                  <p className="uppercase py-3">
                    Valid from{" "}
                    <span className="text-xl text-golden">
          ...
          <div
            className="review-section mt-5 content-rightBox white-bg"
            style={{
              backgroundImage: 'url("/new/assets/img/nominate-hotel-bg.png")',
              backgroundSize: "cover",
              backgroundPosition: "center center",
          ...
          >
            <div className="review-btns">
                        ...
                        <div className="flex items-center gap-3">
                          {(() => {
                            ...
                            const average_rating =
                              review_details &&
                              review_details.reviews?.length > 0
                                ? (overall_rating /
                                    review_details?.review.length) *
                                  5
                                : 0;
                        ...
                        <div className="user-reviews bg-gray-100 p-4 rounded-lg mb-2">
                          <div className="leftSide-reviews">
                            ...
                            <p className="hotel-i">
                              ...
                              <span>Two Double Room</span>
                            </p>
                            <p className="hotel-i">
                              {" "}
                              <Icon className="hotel-i-svg" icon="calendar" />
                              <span>
                                Stayed in{" "}
                                {moment(item?.formDate).format("MMMM DD, YYYY")}
                              </span>
                            </p>
                            <p className="hotel-i">
                              {" "}
                              <Icon className="hotel-i-svg" icon="family" />
                              <span>Family</span>
                            </p>
                            {/* <p className="hotel-i">
                          ...
                          <div className="rightSide-reviews">
                            <div className="">
                              <div className="flex items-center mb-2">
                                <div>
                                  <span className="text-[#9e7922] font-bold text-xl">
              ...
              <a
                href="#"
                data-bs-toggle="offcanvas"
                data-bs-target="#bestHotels"
                aria-controls="bestHotels"
                className=" review-btn drawer-button tab font-bold btn uppercase text-golden hover:bg-transparent border-0 rounded-none hover:border-b-2 hover:border-golden bg-white -z-0 mr-4"
              >
            ...
            <h1 className="text-xl my-8 uppercase text-golden">
              Guest Reviews ({review_details?.length})
            </h1>
            <div className="row">
              <div className="col-md-4 mt-3">
                <div className="d-flex align-items-center gap-3">
                  <p className="rateBoxNumber mb-0">7.9/5</p>
                  <span>Good</span>
                </div>
              </div>
                  ...
                  return (
                    <div key={it._id} className="col-md-4">
                      ...
                      >
                        ...
                        <p className="mb-0 mt-2">{it?.review}</p>
                      </div>
                    </div>
                  );
                })}
          ...
          >
            <h1
              className=" mb-3 text-golden uppercase"
            ...
            <div className="contect-info flex items-center gap-4">
              ...
              <p className="text-md flex info-p">
                ...
                {new_fetch_hotel_info?.hotel?.hotel_manager_telephone}
              </p>
              <p className="text-md flex info-p">
                <span className="text-golden">
                  <svg
            ...
            <div className="w-full my-3 flex flex-col md:flex-row justify-between items-center gap-5">
              <div className="flex gap-3">
                ...
                <button
                  className="px-7 py-2 rounded border border-golden text-golden "
                  onClick={() => (window.href.location = hotel?.website)}
                >
                  Visit Our Website
                </button>
              </div>
            ...
            <h3
              className="uppercase  my-3 text-golden"
              style={{ fontWeight: "500", fontSize: "17px" }}
            >
              Share This
            </h3>
